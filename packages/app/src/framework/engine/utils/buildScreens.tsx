...
const ChildrenStack = (screens: Route["screens"], name: string, config: RouteConfig) => {
  ...
  const Screens: FC = (props) => (
    <RootChildren.Navigator
      key={name}
      initialRouteName={screens[name]?.initialRouteName}
      screenOptions={({ route }) => getConfig(screens[name], route, config.icons, config.ui)}
      {...props}
    >
      {ChildrenScreens}
    </RootChildren.Navigator>
...
export const buildScreens = ({ screens, rootType }: Route, config: RouteConfig) => {
  const Root = getNavigator(rootType);
   
  return Object.keys(screens).map((value, index) => {
    if (config?.isAuth && screens[value].unMountIfIsAuthenticated) {
      return null
    }
    if (screens[value].type !== NavigatorType.component) {
      const Screens = ChildrenStack(screens, value, config);

      return (
        <Root.Screen
          key={`${value}-${index}`}
          name={value}
          listeners={screens[value]?.options?.preventDefault && preventDefault}
      ...
      const Screen = (props: any) => {
        return <ExtensionComponent {...props} screenProps={screens[value]} />
      }
